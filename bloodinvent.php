...
// --- 1. XỬ LÝ FORM (ADD / UPDATE / DELETE) ---
...
// B. CẬP NHẬT (UPDATE)
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'update_unit') {
    ...
    $sql = "UPDATE bloodunit SET volume=?, collectionDate=?, expiryDate=?, storageLocation=? WHERE id=?";
    
    if ($stmt = mysqli_prepare($link, $sql)) {
        mysqli_stmt_bind_param($stmt, "isssi", $vol, $col_date, $exp_date, $location, $id);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
        echo "<script>alert('Unit updated successfully!'); window.location.href='bloodinvent.php';</script>";
        exit();
    }
}
...
if (isset($_POST['action']) && $_POST['action'] == 'delete_unit') {
    $id = intval($_POST['unit_id']);
    // Xóa cứng (Hard Delete) vì đây là quản lý kho, hoặc có thể đổi thành soft delete nếu muốn
    $sql = "DELETE FROM bloodunit WHERE id = ?";
    if ($stmt = mysqli_prepare($link, $sql)) {
        mysqli_stmt_bind_param($stmt, "i", $id);
        mysqli_stmt_execute($stmt);
        echo "<script>alert('Unit deleted successfully!'); window.location.href='bloodinvent.php';</script>";
        exit();
    }
}
...
while ($row = mysqli_fetch_assoc($res_all)) {
    ...
    // Format date cho input date html (Y-m-d) và hiển thị (d/m/Y)
    $row['col_date_raw'] = date('Y-m-d', strtotime($row['collectionDate']));
    $row['exp_date_raw'] = date('Y-m-d', strtotime($row['expiryDate']));
...
<body class="bg-[#F3F4F6] dark:bg-dark text-gray-800 dark:text-gray-200 font-sans transition-colors duration-300">
    ...
    <div class="flex h-screen overflow-hidden">
            ...
            <main class="flex-1 overflow-y-auto p-8 bg-[#F3F4F6] dark:bg-dark">
                ...
                <div class="bg-white dark:bg-gray-dark rounded-xl shadow-lg overflow-hidden border border-gray-100 dark:border-gray-700">
                    ...
                    <div class="flex flex-col">
                        <?php $idx=0; foreach ($inventory as $group => $data): $idx++; ?>
                        <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-700 flex items-center hover:bg-gray-50 dark:hover:bg-gray-800 transition group">
                            <div class="w-12 text-center text-gray-400 font-medium"><?php echo $idx; ?></div>
                            <div class="flex-1 grid grid-cols-6 gap-4 text-center items-center">
                                <div class="text-left pl-4">
                                    ...
                                    <div class="text-xs text-gray-500 mt-1">Managed</div>
                                </div>
                                ...
                                <div class="flex items-center justify-center">
                                    <button onclick="openDetailsModal('<?php echo $group; ?>')" class="text-sm font-bold text-white bg-[#D85555] hover:bg-[#B91C1C] px-4 py-2 rounded-lg transition shadow-sm">
                                        View & Manage
                                    </button>
                                </div>
                            </div>
                    ...
                    <form method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <div>
                                ...
                                <select name="volume" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-red-500 outline-none bg-white">
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <div><label class="block text-sm font-semibold text-gray-600 mb-2">Collection Date</label><input type="date" name="collection_date" id="colDate" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-red-500 outline-none" required></div>
                            <div><label class="block text-sm font-semibold text-gray-600 mb-2">Expiry Date</label><input type="date" name="expiry_date" id="expDate" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-red-500 outline-none" required></div>
                        </div>

                        <div><label class="block text-sm font-semibold text-gray-600 mb-2">Storage Location</label><input type="text" name="storage_location" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-red-500 outline-none" required></div>

                        <div class="flex justify-end gap-3 pt-4">
                            <button type="button" onclick="closeAddUnitModal()" class="px-6 py-2.5 rounded-lg bg-[#1a2231] text-white font-medium">Cancel</button>
                            <button type="submit" class="px-6 py-2.5 rounded-lg bg-[#DC2626] text-white font-bold hover:bg-red-700">Create Unit</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div id="editUnitModal" class="hidden fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm transition-all duration-300">
            <div class="bg-white w-full max-w-2xl rounded-xl shadow-2xl overflow-hidden transform scale-95 opacity-0 transition-all duration-300" id="editUnitContent">
                <div class="bg-blue-600 px-6 py-4 flex justify-between items-center text-white">
                    <h3 class="text-lg font-bold">Edit Unit</h3>
                    <button onclick="closeEditUnitModal()" class="text-white/80 hover:text-white bg-black/10 hover:bg-black/20 rounded-full p-1"><svg width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
                </div>
                <div class="p-8">
                    <form method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <div>
                                <label class="block text-sm font-semibold text-gray-600 mb-2">Volume (ml)</label>
                                <select name="volume" id="editVolume" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 outline-none bg-white">
                                    <option value="250">250 ml</option>
                                    <option value="350">350 ml</option>
                                    <option value="450">450 ml</option>
                                </select>
                            ...
                            <div>
                                <label class="block text-sm font-semibold text-gray-600 mb-2">Blood Type (Read Only)</label>
                                <input type="text" id="editDisplayType" disabled class="w-full border border-gray-300 bg-gray-50 text-gray-500 rounded-lg px-4 py-3 outline-none">
                            </div>
                        </div>

                        <div class="grid grid-cols-2 gap-6">
                            <div><label class="block text-sm font-semibold text-gray-600 mb-2">Collection Date</label><input type="date" name="collection_date" id="editColDate" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 outline-none" required></div>
                            <div><label class="block text-sm font-semibold text-gray-600 mb-2">Expiry Date</label><input type="date" name="expiry_date" id="editExpDate" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 outline-none" required></div>
                        </div>

                        <div><label class="block text-sm font-semibold text-gray-600 mb-2">Storage Location</label><input type="text" name="storage_location" id="editLocation" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 outline-none" required></div>

                        <div class="flex justify-end gap-3 pt-4">
                            <button type="button" onclick="closeEditUnitModal()" class="px-6 py-2.5 rounded-lg bg-gray-600 text-white font-medium">Cancel</button>
                            <button type="submit" class="px-6 py-2.5 rounded-lg bg-blue-600 text-white font-bold hover:bg-blue-700">Update Unit</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div id="detailsModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div class="bg-white dark:bg-gray-800 w-full max-w-6xl rounded-2xl shadow-2xl transform scale-95 opacity-0 transition-all duration-300 border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col max-h-[90vh]" id="detailsModalContent">
                ...
                <div class="p-6 grid grid-cols-4 gap-4 bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 shrink-0">
                    <div class="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm">
                        ...
                        <h4 class="text-xl font-extrabold text-gray-800 dark:text-white mt-1" id="statTotal">0</h4>
                    </div>
                    <div class="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm">
                        <p class="text-xs text-gray-500 uppercase font-semibold">Total Volume</p>
                        <h4 class="text-xl font-extrabold text-gray-800 dark:text-white mt-1"><span id="statVol">0</span> ml</h4>
                    </div>
                    <div class="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm">
                        <p class="text-xs text-gray-500 uppercase font-semibold">Expiring Soon</p>
                        <h4 class="text-xl font-extrabold text-gray-800 dark:text-white mt-1" id="statExpiring">0</h4>
                    </div>
                     <div class="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm">
                        <p class="text-xs text-gray-500 uppercase font-semibold">Expired</p>
                        <h4 class="text-xl font-extrabold text-red-600 mt-1" id="statExpired">0</h4>
                    </div>
                </div>
                <div class="px-6 py-4 flex justify-between items-center bg-white dark:bg-gray-800 shrink-0 border-b border-gray-100 dark:border-gray-700">
                    ...
                    <button onclick="openAddUnitModal()" class="bg-[#CF2222] hover:bg-red-700 text-white px-4 py-2 rounded-lg text-sm font-bold shadow-sm transition flex items-center gap-2"><svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg> Add Unit</button>
                </div>
                <div class="flex-1 overflow-y-auto px-6 pb-6">
                    <table class="w-full text-left border-collapse">
                        <thead class="bg-gray-50 dark:bg-gray-900 text-xs font-bold text-gray-400 uppercase tracking-wider sticky top-0 z-10">
                            <tr><th class="py-3 px-4">ID</th><th class="py-3 px-4">Vol</th><th class="py-3 px-4">Collected</th><th class="py-3 px-4">Expires</th><th class="py-3 px-4">Location</th><th class="py-3 px-4">Status</th><th class="py-3 px-4 text-center">Action</th></tr>
                        </thead>
    ...
    <script>
        ...
        function openAddUnitModal() {
            ...
            document.getElementById('displayRhType').value = rh;
            ...
            setTimeout(() => { addUnitContent.classList.remove('opacity-0', 'scale-95'); addUnitContent.classList.add('opacity-100', 'scale-100'); }, 10);
        }
        function closeAddUnitModal() {
            addUnitContent.classList.remove('opacity-100', 'scale-100'); addUnitContent.classList.add('opacity-0', 'scale-95');
        ...
        document.getElementById('colDate').addEventListener('change', function() {
            ...
            if (!isNaN(colDate)) { colDate.setDate(colDate.getDate() + 35); document.getElementById('expDate').value = colDate.toISOString().split('T')[0]; }
        });

        // --- EDIT UNIT MODAL ---
        const editUnitModal = document.getElementById('editUnitModal');
        const editUnitContent = document.getElementById('editUnitContent');
        ...
        function openEditModal(id) {
            ...
            document.getElementById('editVolume').value = unit.volume;
            document.getElementById('editDisplayType').value = unit.group_key;
            document.getElementById('editColDate').value = unit.col_date_raw;
            document.getElementById('editExpDate').value = unit.exp_date_raw;
            document.getElementById('editLocation').value = unit.storageLocation;

            editUnitModal.classList.remove('hidden');
            setTimeout(() => { editUnitContent.classList.remove('opacity-0', 'scale-95'); editUnitContent.classList.add('opacity-100', 'scale-100'); }, 10);
        }
        function closeEditUnitModal() {
            editUnitContent.classList.remove('opacity-100', 'scale-100'); editUnitContent.classList.add('opacity-0', 'scale-95');
            setTimeout(() => { editUnitModal.classList.add('hidden'); }, 300);
        }

        // --- DELETE UNIT LOGIC ---
        function confirmDeleteUnit(id) {
            if(confirm('Are you sure you want to delete Unit #' + id + '? This cannot be undone.')) {
                document.getElementById('deleteUnitId').value = id;
                document.getElementById('deleteForm').submit();
            }
        }

        // --- DETAILS MODAL & TABLE RENDER ---
        ...
        function updateDetailsView() {
            ...
            document.getElementById('statExpired').innerText = allGroupUnits.filter(u => u.status === 'Expired').length;

            // Render Table (Có thêm nút Action)
            tableBody.innerHTML = '';
            if (units.length === 0) {
                tableBody.innerHTML = '<tr><td colspan="7" class="p-6 text-center text-gray-500">No units found matching criteria.</td></tr>';
            } else {
                units.forEach(u => {
                    const row = `
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 border-b border-gray-100 dark:border-gray-700 transition">
                            ...
                            <td class="py-3 px-4 text-sm">${u.volume} ml</td>
                            <td class="py-3 px-4 text-sm text-gray-500">${u.col_date_fmt}</td>
                            <td class="py-3 px-4 text-sm text-gray-500">${u.exp_date_fmt}</td>
                            <td class="py-3 px-4 text-sm font-medium">${u.storageLocation}</td>
                            <td class="py-3 px-4"><span class="px-2.5 py-1 rounded-md text-xs font-bold ${u.css}">${u.status}</span></td>
                            <td class="py-3 px-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <button onclick="openEditModal(${u.id})" class="text-blue-500 hover:bg-blue-50 p-1.5 rounded-full" title="Edit Unit"><svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></button>
                                    <button onclick="confirmDeleteUnit(${u.id})" class="text-red-500 hover:bg-red-50 p-1.5 rounded-full" title="Delete Unit"><svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>
                                </div>
                            </td>
                        </tr>`;
